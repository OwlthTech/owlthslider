...
function os_remove_default_types_effects_meta_box()
{
    remove_meta_box('os_slider_typediv', 'os_slider', 'side');
    remove_meta_box('os_slider_effectdiv', 'os_slider', 'side');
}
add_action('add_meta_boxes', 'os_remove_default_types_effects_meta_box');

/**
 * Register Sliders.
 */

function os_add_types_effects_meta_box()
{
    ...
    if ($post && $post->post_type === 'os_slider') {
        ...
        // Example slider type slugs: 'default', 'carousel', 'reviews'
        if (in_array('default', $slider_types) || in_array('carousel', $slider_types)) {
            // Table
            add_meta_box(
                'os_slider_details', 
        ...
        } elseif (in_array('reviews', $slider_types)) {
            add_meta_box(
                'os_slider_reviews_settings',
                __('Reviews Slider Settings', 'owlthslider'),
        ...
        // Add more conditions for other slider types as needed
    }
}
add_action('add_meta_boxes', 'os_add_types_effects_meta_box');
...
function render_table_rows($index, $data = [])
{
    ...
    <tr>
        ...
        <td class="action-column">
            <button type="button" class="button-icon remove-row" title="Remove Slide"><span
                    class="dashicons dashicons-trash"></span></button>
            <button type="button" class="button-icon duplicate-row" title="Duplicate Slide"><span
...
function os_slider_render_reviews_settings($post) {
    ...
    <p>
        ...
        <input type="text" id="os_slider_google_place_id" name="os_slider_options[os_slider_google_place_id]" value="<?php echo esc_attr($options['os_slider_google_place_id']); ?>" required />
    </p>
    <p>
 ...
 */
function os_render_reviews_table($post_id, $google_place_id) {
    ...
    $reviews = os_fetch_google_reviews($google_place_id);
...
function os_slider_render_options($post)
{
    ...
    <div class="os-slider-settings">
        ...
        <p>
            ...
            <input type="number" id="os_slider_speed" name="os_slider_options[os_slider_speed]" value="<?php echo esc_attr($options['os_slider_speed']); ?>" min="1" />
        </p>
        ...
        <p>
            ...
            <select id="os_slider_align" name="os_slider_options[os_slider_align]">
                <option value="start" <?php selected($options['os_slider_align'], 'start'); ?>><?php _e('Start', 'owlthslider'); ?></option>
                <option value="center" <?php selected($options['os_slider_align'], 'center'); ?>><?php _e('Center', 'owlthslider'); ?></option>
                <option value="end" <?php selected($options['os_slider_align'], 'end'); ?>><?php _e('End', 'owlthslider'); ?></option>
            </select>
        </p>
        ...
        <p>
            <label>
                <input type="checkbox" id="os_slider_autoplay" name="os_slider_options[os_slider_autoplay]" value="yes" <?php checked($options['os_slider_autoplay'], 'yes'); ?> />
        ...
        <div id="autoplay_options" style="<?php echo ($options['os_slider_autoplay'] === 'yes') ? '' : 'display:none;'; ?>">
            <p>
                ...
                <input type="number" id="os_slider_autoplay_delay" name="os_slider_options[os_slider_autoplay_delay]" value="<?php echo esc_attr($options['os_slider_autoplay_delay']); ?>" min="0" />
            </p>
        </div>

        <p>
            <label>
                <input type="checkbox" id="os_slider_autoscroll" name="os_slider_options[os_slider_autoscroll]" value="yes" <?php checked($options['os_slider_autoscroll'], 'yes'); ?> />
                <?php _e('Enable Autoscroll', 'owlthslider'); ?>
            </label>
        </p>

        <div id="autoscroll_options" style="<?php echo ($options['os_slider_autoscroll'] === 'yes') ? '' : 'display:none;'; ?>">
            <p>
                <label for="os_slider_autoscroll_speed"><?php _e('Autoscroll Speed', 'owlthslider'); ?></label><br />
                <input type="number" id="os_slider_autoscroll_speed" name="os_slider_options[os_slider_autoscroll_speed]" value="<?php echo esc_attr($options['os_slider_autoscroll_speed']); ?>" min="1" />
            </p>
        </div>
